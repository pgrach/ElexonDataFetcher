...
async function main(): Promise<void> {
  console.log(`=== Staged Reingest for March 21, 2025 (Periods ${START_PERIOD}-${END_PERIOD}) ===`);
  ...
  try {
    ...
    if (missingPeriods.length > 0) {
      ...
      const nextEnd = Math.min(nextStart + 5, missingPeriods[missingPeriods.length - 1]);
