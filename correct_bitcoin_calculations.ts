/**
 * Corrected Bitcoin Calculation Updater
 * 
 * This script correctly calculates Bitcoin mined for a specific date
 * and updates the monthly and yearly summaries accordingly, using the correct formula.
 * 
 * Usage:
 *   npx tsx correct_bitcoin_calculations.ts <date>
 * 
 * Example:
 *   npx tsx correct_bitcoin_calculations.ts 2025-03-27
 */
...
// Use the correct difficulty value
...
function calculateBitcoinMined(energyMWh: number, minerModel: string, difficulty: number): number {
  ...
  // Calculate expected Bitcoin mined (this follows the correct calculation formula)
  // Formula: (petahashes * 1e15 / difficulty) * 6.25 BTC per block * number of seconds in 30 minutes
  // Divide by 1e21 to get the correct scale
  const bitcoinMined = (petahashesGenerated * 1e15 / difficulty) * 6.25 * 1800 / 1e21;
