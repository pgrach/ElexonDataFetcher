...
const START_PERIOD = 43; // Change these values to process different batches
const END_PERIOD = 48;  // Example: 1-2, 3-6, 7-10, etc.
...
// First clear the existing data for specific periods only (not the whole day)
async function clearExistingPeriodsData(): Promise<void> {
  ...
  // Delete only the specified periods
  ...
  console.log(`Deleted ${deleteResult.rowCount} curtailment records for specified periods`);
...
async function main(): Promise<void> {
  console.log(`=== Staged Reingest for March 21, 2025 ===`);
  ...
  try {
    ...
    console.log(`\nCompleted periods (${completedPeriods.size}): ${Array.from(completedPeriods).sort((a, b) => a - b).join(', ')}`);
    ...
    console.log(`npx tsx update_march_21_summaries.ts`);
  } catch (error) {
