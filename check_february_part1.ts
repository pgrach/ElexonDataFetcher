/**
 * Complete check of the first 14 days in February 2025 for Beatrice data
 * Part 1 of 2 to verify ALL February days
 */
...
// Generate dates for first half of February 2025
function getFebruaryDates(): string[] {
  const dates: string[] = [];
 ...
 */
async function main() {
  const dates = getFebruaryDates();
  
  console.log("\n=== CHECKING FEBRUARY 2025 (PART 1: FEB 1-14) FOR BEATRICE DATA ===\n");
  console.log(`Dates to check: ${dates.length} days (Feb 1-14, 2025)`);
  console.log(`Periods to check per day: ${PERIODS_TO_CHECK.join(', ')}`);
  console.log(`Total checks: ${dates.length * PERIODS_TO_CHECK.length}`);
  ...
  console.log("\n=== PART 1 SCAN COMPLETE ===");
  console.log(`Checked first 14 days of February 2025`);
  
  if (foundBeatriceData) {
    console.log("\nFOUND BEATRICE DATA IN FIRST HALF OF FEBRUARY 2025");
    console.log("See above for specific dates and periods");
  } else {
    console.log("\nNO BEATRICE DATA FOUND IN FIRST HALF OF FEBRUARY 2025");
    console.log("Checked Feb 1-14 with multiple periods per day");
  }
}
