...
export default function CurtailmentComparisonCard({
  ...
  const percentageDiff =
    curtailmentPayment > 0 ? (difference / curtailmentPayment) * 100 : 0;
        ...
        {isSummaryLoading || isBitcoinLoading ? (
          ...
          <div className="bg-blue-50/30 border border-dashed border-blue-200 p-4 rounded-md flex flex-col items-center">
            ...
            <h3 className="font-semibold text-lg text-blue-500 mb-2">
              No Curtailment Events Today
            </h3>
            <p className="text-sm text-blue-400 text-center mb-3">
              No wind farms were curtailed on this date, meaning all available
              wind energy was utilized by the grid.
            </p>
            <p className="text-sm text-muted-foreground text-center">
              Try selecting a different date to see curtailment events and
          ...
          <>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="flex flex-col">
                ...
                <span className="text-xl font-semibold">
                  £{Math.round(curtailmentPayment).toLocaleString()}
                </span>
              ...
              <div className="flex flex-col">
                ...
                <span
                  className={`text-xl font-semibold ${isBitcoinBetter ? "text-green-500" : ""}`}
                >
                  £{Math.round(bitcoinValue).toLocaleString()}
                </span>
                <span className="text-xs text-muted-foreground">
                  Bitcoin mining value
                </span>
              </div>
            </div>
            ...
            >
              ...
              <p className="text-sm">
                {isBitcoinBetter ? (
                  <>
                    Mining Bitcoin instead of accepting curtailment payments
                    would have generated{" "}
                    <strong>
                      £{Math.abs(Math.round(difference)).toLocaleString()} more
                      value
                    </strong>{" "}
                    ({Math.abs(Math.round(percentageDiff))}% increase) during
                    this period.
                  </>
                ) : difference < 0 ? (
                  <>
                    Curtailment payments were{" "}
                    <strong>
                      £{Math.abs(Math.round(difference)).toLocaleString()}{" "}
                      higher
                    </strong>{" "}
                    than potential Bitcoin mining value during this period.
                  </>
                ) : (
