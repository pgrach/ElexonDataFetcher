/**
 * Complete Reingestion Script
 * 
 * This script performs a comprehensive reingestion of data for a specific date:
 * 1. Clears all existing data for the target date in curtailment_records and historical_bitcoin_calculations
 * 2. Fetches complete data from Elexon API for all 48 settlement periods
 * 3. Recalculates Bitcoin mining potential for all three miner models
 * 4. Updates all daily, monthly, and yearly summary tables
 * 
 * Usage: npx tsx complete_reingestion.ts YYYY-MM-DD
 * Example: npx tsx complete_reingestion.ts 2025-04-02
 */
...
const DEFAULT_TARGET_DATE = format(new Date(), "yyyy-MM-dd");
...
const LOG_FILE_PATH = `./logs/complete_reingestion_${TARGET_DATE}_${format(new Date(), "yyyy-MM-dd'T'HH-mm-ss")}.log`;
...
async function processDailyCurtailment(date: string): Promise<number> {
  try {
    ...
    // Clear all existing records for the target date first
    const deleteResult = await db.delete(curtailmentRecords)
      .where(eq(curtailmentRecords.settlementDate, date));
    
    logger.log(`Cleared all existing curtailment records for ${date}`);
  ...
  try {
    ...
    for (let i = 0; i < batches; i++) {
      ...
      const batchCalculations = batch.map(record => {
        ...
        // Make sure volume is positive and use the correct formula
      ...
      // Insert batch of calculations only if we have data
      if (batchCalculations.length > 0) {
...
async function updateDailySummary(date: string): Promise<void> {
  try {
    ...
    if (curtailmentStats.length > 0 && curtailmentStats[0].totalCurtailedEnergy) {
      // Insert new daily summary - only include fields that exist in the schema
...
async function updateMonthlySummary(yearMonth: string): Promise<void> {
  try {
    ...
    if (monthlyStats.length > 0 && monthlyStats[0].totalCurtailedEnergy) {
      // Insert new monthly summary - only use fields that exist in the schema
...
async function updateYearlySummary(year: string): Promise<void> {
  try {
    ...
    if (yearlyStats.length > 0 && yearlyStats[0].totalCurtailedEnergy) {
      // Insert new yearly summary - only use fields that exist in the schema
