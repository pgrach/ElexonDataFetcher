...
async function processMarch27RemainingPeriods(): Promise<void> {
  try {
    console.log(`\n===== Starting to process remaining periods (${START_PERIOD}-48) for ${TARGET_DATE} =====\n`);
    ...
    for (let period = START_PERIOD; period <= 48; period++) {
      console.log(`\nProcessing period ${period}...`);
      
      try {
        // Execute a server script to process this period
        // This is a simpler approach than reimplementing the full processing logic
        // We'll call the server process_period endpoint
        const command = `npx tsx server/services/curtailment.ts process-period ${TARGET_DATE} ${period}`;
...
async function updateDailySummary(totalVolume: number, totalPayment: number): Promise<void> {
  ...
  try {
    await db.execute(
      ...
      SET 
        total_curtailed_energy = $1,
        total_payment = $2,
        last_updated = NOW()
      WHERE summary_date = $3`,
      [totalVolume.toString(), totalPayment.toString(), TARGET_DATE]
    );
