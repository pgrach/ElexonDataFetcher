 ...
 * historicalBitcoinCalculations tables for all 2025 data, identifying and fixing
...
/**
 * Get all dates in 2025 with curtailment records
 */
async function get2025Dates(): Promise<string[]> {
  console.log('Finding all 2025 dates with curtailment records...');
  ...
  const result = await db
    ...
    .from(curtailmentRecords)
    .where(like(sql`settlement_date::text`, '2025-%'))
  ...
  console.log(`Found ${dates.length} dates in 2025 with curtailment records`);
  
  return dates;
}

/**
 * Main function to reconcile 2025 data
 */
async function reconcile2025Data() {
  try {
    console.log(`=== Starting 2025 Data Reconciliation ===`);
    console.log(`Time range: ${START_DATE} to ${END_DATE}`);
    
    // Get all 2025 dates with curtailment records
    const allDates = await get2025Dates();
    
    if (allDates.length === 0) {
      console.log('No 2025 data found in curtailment_records table.');
      return;
    }
    ...
    if (datesToFix.length > 0) {
      ...
      console.log('\n✓ All 2025 data is properly reconciled! No fixes needed.');
    }
    
    console.log(`\n=== Reconciliation Complete ===`);
  } catch (error) {
    console.error('Error during 2025 data reconciliation:', error);
    throw error;
  }
}
