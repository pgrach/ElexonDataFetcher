/**
 * Fix April 1, 2025 Bitcoin Calculations (Aggregated approach)
 * 
 * This script handles duplicate curtailment records by first aggregating the energy data
 * by settlement_period and farm_id before calculating Bitcoin.
 */
...
const LOG_FILE = path.join(LOG_DIR, `fix_april1_2025_aggregated_${new Date().toISOString().replace(/:/g, '-')}.log`);
...
async function main() {
  try {
    ...
    // First, get the total curtailment energy for this date
