/**
 * Nuclear BMU Mapping Update Script
 * 
 * This script fetches data from the Elexon API to create a mapping of all Nuclear
 * Balancing Mechanism Units (BMUs) in the system.
 * 
 * The resulting JSON file is saved to server/data/nuclearBmuMapping.json
 */
...
async function updateNuclearBmuMapping() {
  try {
    console.log('Starting Nuclear BMU mapping update process...');

    // Create data directory if it doesn't exist
    await fs.mkdir(path.dirname(NUCLEAR_BMU_MAPPING_PATH), { recursive: true });
    ...
    // Filter Nuclear units
    const nuclearBmus = bmuData.filter(bmu => 
      bmu?.fuelType?.toUpperCase() === 'NUCLEAR'
    );
    
    console.log(`Found ${nuclearBmus.length} Nuclear BMUs before validation`);

    // Validate and process Nuclear units
    const validNuclearBmus = (await Promise.all(
      nuclearBmus
        .filter(bmu => {
        ...
        .map(async bmu => {
          try {
            ...
            // Try to determine reactor type from the name (if possible)
            let reactorType = 'Unknown';
            if (name.includes('agr')) {
              reactorType = 'Advanced Gas-cooled Reactor (AGR)';
            } else if (name.includes('pwr')) {
              reactorType = 'Pressurized Water Reactor (PWR)';
            } else if (name.includes('magnox')) {
              reactorType = 'Magnox';
            }
            ...
            return {
              ...
              fuelType: bmu.fuelType || 'NUCLEAR',
              reactorType: reactorType, 
              leadPartyName: bmu.leadPartyName || 'Unknown',
              region: region
            };
          } catch (error) {
            console.warn('Nuclear BMU validation failed:', error);
            return null;
          }
        })
    )).filter(bmu => bmu !== null);
    ...
    const sortedNuclearBmus = validNuclearBmus.sort((a, b) => 
      a!.elexonBmUnit.localeCompare(b!.elexonBmUnit)
    );

    console.log(`Found ${sortedNuclearBmus.length} valid Nuclear BMUs`);

    // Write the filtered data to nuclearBmuMapping.json
    await fs.writeFile(
      NUCLEAR_BMU_MAPPING_PATH,
      JSON.stringify(sortedNuclearBmus, null, 2),
      'utf8'
    );

    console.log(`Successfully updated Nuclear BMU mapping at ${NUCLEAR_BMU_MAPPING_PATH}`);
    console.log(`Total Nuclear BMUs: ${sortedNuclearBmus.length}`);

  } catch (error) {
    console.error('Error updating Nuclear BMU mapping:', error);
    process.exit(1);
  }
}

// Execute the update function
updateNuclearBmuMapping();