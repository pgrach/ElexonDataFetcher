/**
 * Coal BMU Mapping Update Script
 * 
 * This script fetches data from the Elexon API to create a mapping of all Coal
 * Balancing Mechanism Units (BMUs) in the system.
 * 
 * The resulting JSON file is saved to server/data/coalBmuMapping.json
 */
...
async function updateCoalBmuMapping() {
  try {
    console.log('Starting Coal BMU mapping update process...');

    // Create data directory if it doesn't exist
    await fs.mkdir(path.dirname(COAL_BMU_MAPPING_PATH), { recursive: true });
    ...
    // Filter Coal units
    const coalBmus = bmuData.filter(bmu => 
      bmu?.fuelType?.toUpperCase() === 'COAL'
    );
    
    console.log(`Found ${coalBmus.length} Coal BMUs before validation`);

    // Validate and process Coal units
    const validCoalBmus = (await Promise.all(
      coalBmus
        .filter(bmu => {
        ...
        .map(async bmu => {
          try {
            ...
            return {
              ...
              fuelType: bmu.fuelType || 'COAL',
              leadPartyName: bmu.leadPartyName || 'Unknown',
              region: region
            };
          } catch (error) {
            console.warn('Coal BMU validation failed:', error);
            return null;
          }
        })
    )).filter(bmu => bmu !== null);
    ...
    const sortedCoalBmus = validCoalBmus.sort((a, b) => 
      a!.elexonBmUnit.localeCompare(b!.elexonBmUnit)
    );

    console.log(`Found ${sortedCoalBmus.length} valid Coal BMUs`);

    // Write the filtered data to coalBmuMapping.json
    await fs.writeFile(
      COAL_BMU_MAPPING_PATH,
      JSON.stringify(sortedCoalBmus, null, 2),
      'utf8'
    );

    console.log(`Successfully updated Coal BMU mapping at ${COAL_BMU_MAPPING_PATH}`);
    console.log(`Total Coal BMUs: ${sortedCoalBmus.length}`);

  } catch (error) {
    console.error('Error updating Coal BMU mapping:', error);
    process.exit(1);
  }
}

// Execute the update function
updateCoalBmuMapping();