/**
 * OCGT BMU Mapping Update Script
 * 
 * This script fetches data from the Elexon API to create a mapping of all OCGT
 * (Open Cycle Gas Turbine) Balancing Mechanism Units (BMUs) in the system.
 * 
 * The resulting JSON file is saved to server/data/ocgtBmuMapping.json
 */
...
const OCGT_BMU_MAPPING_PATH = path.join(process.cwd(), 'server', 'data', 'ocgtBmuMapping.json');
...
async function updateOcgtBmuMapping() {
  try {
    console.log('Starting OCGT BMU mapping update process...');

    // Create data directory if it doesn't exist
    await fs.mkdir(path.dirname(OCGT_BMU_MAPPING_PATH), { recursive: true });
    ...
    // Filter OCGT units
    const ocgtBmus = bmuData.filter(bmu => 
      bmu?.fuelType?.toUpperCase() === 'OCGT'
    );
    
    console.log(`Found ${ocgtBmus.length} OCGT BMUs before validation`);

    // Validate and process OCGT units
    const validOcgtBmus = (await Promise.all(
      ocgtBmus
        .filter(bmu => {
        ...
        .map(async bmu => {
          try {
            ...
            console.warn('OCGT BMU validation failed:', error);
            return null;
          }
        })
    )).filter(bmu => bmu !== null);
    ...
    const sortedOcgtBmus = validOcgtBmus.sort((a, b) => 
      a!.elexonBmUnit.localeCompare(b!.elexonBmUnit)
    );

    console.log(`Found ${sortedOcgtBmus.length} valid OCGT BMUs`);

    // Write the filtered data to ocgtBmuMapping.json
    await fs.writeFile(
      OCGT_BMU_MAPPING_PATH,
      JSON.stringify(sortedOcgtBmus, null, 2),
      'utf8'
    );

    console.log(`Successfully updated OCGT BMU mapping at ${OCGT_BMU_MAPPING_PATH}`);
    console.log(`Total OCGT BMUs: ${sortedOcgtBmus.length}`);

  } catch (error) {
    console.error('Error updating OCGT BMU mapping:', error);
    process.exit(1);
  }
}

// Execute the update function
updateOcgtBmuMapping();