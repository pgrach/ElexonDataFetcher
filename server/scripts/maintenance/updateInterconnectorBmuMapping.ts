/**
 * Interconnector BMU Mapping Update Script
 * 
 * This script fetches data from the Elexon API to create a mapping of all Interconnector
 * Balancing Mechanism Units (BMUs) in the system.
 * 
 * The resulting JSON file is saved to server/data/interconnectorBmuMapping.json
 */
...
const INTERCONNECTOR_BMU_MAPPING_PATH = path.join(process.cwd(), 'server', 'data', 'interconnectorBmuMapping.json');
...
// Map of interconnector fuel types to destination countries
const interconnectorMapping: Record<string, string> = {
  'INTELEC': 'IFA (France)',
  'INTEW': 'East-West (Ireland)',
  'INTFR': 'IFA2 (France)',
  'INTGRNL': 'BritNed (Netherlands)',
  'INTIFA2': 'IFA2 (France)',
...
async function updateInterconnectorBmuMapping() {
  try {
    console.log('Starting Interconnector BMU mapping update process...');

    // Create data directory if it doesn't exist
    await fs.mkdir(path.dirname(INTERCONNECTOR_BMU_MAPPING_PATH), { recursive: true });
    ...
    // Filter Interconnector units
    const interconnectorBmus = bmuData.filter(bmu => 
      bmu?.fuelType?.toUpperCase().startsWith('INT')
    );
    
    console.log(`Found ${interconnectorBmus.length} Interconnector BMUs before validation`);

    // Validate and process Interconnector units
    const validInterconnectorBmus = (await Promise.all(
      interconnectorBmus
        .filter(bmu => {
          // Ensure the BMU has required fields
          return bmu.nationalGridBmUnit && bmu.elexonBmUnit && bmu.bmUnitName;
        })
        .map(async bmu => {
          try {
            // Make sure all required fields exist
            if (!bmu.nationalGridBmUnit || !bmu.elexonBmUnit || !bmu.bmUnitName) {
              console.warn('Skipping BMU with missing required fields', bmu.elexonBmUnit || 'unknown');
              return null;
            }

            // Determine connected country from fuel type
            const fuelType = bmu.fuelType?.toUpperCase() || '';
            const connectedCountry = interconnectorMapping[fuelType] || 'Unknown';
            
            return {
              nationalGridBmUnit: bmu.nationalGridBmUnit,
              elexonBmUnit: bmu.elexonBmUnit,
              bmUnitName: bmu.bmUnitName,
              generationCapacity: bmu.generationCapacity || '0.000',
              fuelType: bmu.fuelType || '',
              interconnectorName: connectedCountry,
              leadPartyName: bmu.leadPartyName || 'Unknown'
            };
          } catch (error) {
            console.warn('Interconnector BMU validation failed:', error);
            return null;
          }
        })
    )).filter(bmu => bmu !== null);
    ...
    const sortedInterconnectorBmus = validInterconnectorBmus.sort((a, b) => 
      a!.elexonBmUnit.localeCompare(b!.elexonBmUnit)
    );

    console.log(`Found ${sortedInterconnectorBmus.length} valid Interconnector BMUs`);

    // Write the filtered data to interconnectorBmuMapping.json
    await fs.writeFile(
      INTERCONNECTOR_BMU_MAPPING_PATH,
      JSON.stringify(sortedInterconnectorBmus, null, 2),
      'utf8'
    );

    console.log(`Successfully updated Interconnector BMU mapping at ${INTERCONNECTOR_BMU_MAPPING_PATH}`);
    console.log(`Total Interconnector BMUs: ${sortedInterconnectorBmus.length}`);

  } catch (error) {
    console.error('Error updating Interconnector BMU mapping:', error);
    process.exit(1);
  }
}

// Execute the update function
updateInterconnectorBmuMapping();