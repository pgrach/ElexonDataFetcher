/**
 * CCGT BMU Mapping Update Script
 * 
 * This script fetches data from the Elexon API to create a mapping of all CCGT
 * (Combined Cycle Gas Turbine) Balancing Mechanism Units (BMUs) in the system.
 * 
 * The resulting JSON file is saved to server/data/ccgtBmuMapping.json
 */
...
async function updateCcgtBmuMapping() {
  try {
    console.log('Starting CCGT BMU mapping update process...');

    // Create data directory if it doesn't exist
    await fs.mkdir(path.dirname(CCGT_BMU_MAPPING_PATH), { recursive: true });
    ...
    // Filter CCGT units
    const ccgtBmus = bmuData.filter(bmu => 
      bmu?.fuelType?.toUpperCase() === 'CCGT'
    );
    
    console.log(`Found ${ccgtBmus.length} CCGT BMUs before validation`);

    // Validate and process CCGT units
    const validCcgtBmus = (await Promise.all(
      ccgtBmus
        .filter(bmu => {
        ...
        .map(async bmu => {
          try {
            ...
            console.warn('CCGT BMU validation failed:', error);
            return null;
          }
        })
    )).filter(bmu => bmu !== null);
    ...
    const sortedCcgtBmus = validCcgtBmus.sort((a, b) => 
      a!.elexonBmUnit.localeCompare(b!.elexonBmUnit)
    );

    console.log(`Found ${sortedCcgtBmus.length} valid CCGT BMUs`);

    // Write the filtered data to ccgtBmuMapping.json
    await fs.writeFile(
      CCGT_BMU_MAPPING_PATH,
      JSON.stringify(sortedCcgtBmus, null, 2),
      'utf8'
    );

    console.log(`Successfully updated CCGT BMU mapping at ${CCGT_BMU_MAPPING_PATH}`);
    console.log(`Total CCGT BMUs: ${sortedCcgtBmus.length}`);

  } catch (error) {
    console.error('Error updating CCGT BMU mapping:', error);
    process.exit(1);
  }
}

// Execute the update function
updateCcgtBmuMapping();