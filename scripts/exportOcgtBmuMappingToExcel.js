/**
 * Export OCGT BMU Mapping to Excel/CSV
 * 
 * This script converts the JSON OCGT BMU mapping file to CSV format
...
const OUTPUT_PATH = path.join(process.cwd(), 'ocgt_bmu_mapping.csv');

async function exportToCSV() {
  try {
    console.log('Reading OCGT BMU mapping from:', OCGT_BMU_MAPPING_PATH);
    
    // Read the JSON file
    const mappingContent = fs.readFileSync(OCGT_BMU_MAPPING_PATH, 'utf8');
    const bmuMapping = JSON.parse(mappingContent);
    
    console.log(`Loaded ${bmuMapping.length} OCGT BMU mappings`);
    ...
    console.log(`Successfully exported OCGT BMU mapping to CSV at ${OUTPUT_PATH}`);
    console.log('You can now download this file and open it in Excel');
    
  } catch (error) {
    console.error('Error exporting OCGT BMU mapping to CSV:', error);
  }
}
