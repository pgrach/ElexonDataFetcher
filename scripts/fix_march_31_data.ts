/**
 * Fix Data for 2025-03-31
 * 
 * This script fixes the incorrect/incomplete data for March 31, 2025 by:
 * 1. Deleting existing curtailment records for 2025-03-31
 * 2. Reingesting data from Elexon API for all 48 periods
 * 3. Updating all dependent tables in cascade:
 *    - daily_summaries
 *    - monthly_summaries 
 *    - yearly_summaries
...
const LOG_FILE = `./logs/complete_march_27_${new Date().toISOString().replace(/:/g, '-')}.log`;
...
/**
 * Reset curtailment records for the target date
 */
async function resetCurtailmentRecords(): Promise<void> {
  await logMessage(`Clearing existing curtailment records for ${TARGET_DATE}...`);
  ...
  await logMessage(`Found ${beforeCount[0]?.count || 0} existing records to delete`);
  
  // Delete all records for the target date
  await db
  ...
  await logMessage(`Verified deletion: ${afterCount[0]?.count || 0} records remaining`);
}
...
/**
 * Reingest data from Elexon API
 */
async function reingestData(): Promise<void> {
  ...
  try {
    ...
    await logMessage(`Successfully reingested data: ${records[0]?.count || 0} curtailment records created`);
  } catch (error) {
...
async function calculateBitcoinPotential(): Promise<void> {
  ...
  try {
    // Get farm data for the date
    const farmData = await getFarmDataForDate(TARGET_DATE);
    await logMessage(`Processing Bitcoin calculations for ${farmData.length} farms`);
...
async function verifyDataIntegrity(): Promise<void> {
  ...
  try {
    ...
    if (missingPeriods.length > 0) {
      const missingPeriodRanges = getMissingRanges(missingPeriods);
      await logMessage(`Note: ${missingPeriods.length} settlement periods have no curtailment data: ${missingPeriodRanges}`);
...
// Helper function to get ranges of missing periods
function getMissingRanges(periods: number[]): string {
...
async function main() {
  try {
    ...
    await resetCurtailmentRecords();
    await resetBitcoinCalculations();
    
    // Step 2: Reingest data from Elexon API
    await reingestData();
