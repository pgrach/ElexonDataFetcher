/**
 * Manual Bitcoin Calculation Updater
 * 
 * This script manually updates Bitcoin calculations for a specific date
 * when the automated reconciliation processes time out or fail.
 * 
 * Usage:
 *   npx tsx update_bitcoin_calculations.ts <date>
 * 
 * Example:
 *   npx tsx update_bitcoin_calculations.ts 2025-03-27
 */
...
const DEFAULT_DIFFICULTY = 84598642601281; // Default difficulty if not found
...
async function processDate(date: string): Promise<void> {
  ...
  try {
    // 1. Clear any existing calculations for this date
    ...
    console.log(`Cleared ${deletedCount.length} existing calculations`);
    
    // 2. Get all curtailment records for this date
    ...
    if (records.length === 0) {
      console.log(`No records found for ${date}. Exiting.`);
      return;
    }
    
    // 3. Group records by period and farm for efficiency
    const recordsByPeriodAndFarm: Record<string, any[]> = {};
    
    for (const record of records) {
      const key = `${record.settlementPeriod}_${record.farmId}`;
      if (!recordsByPeriodAndFarm[key]) {
        recordsByPeriodAndFarm[key] = [];
      }
      recordsByPeriodAndFarm[key].push(record);
    }
    
    // 4. Process each group and calculate Bitcoin for each miner model
    let totalInserted = 0;
    ...
    for (const key in recordsByPeriodAndFarm) {
      const recordsForGroup = recordsByPeriodAndFarm[key];
      
      if (recordsForGroup.length === 0) continue;
      
      const period = recordsForGroup[0].settlementPeriod;
      const farmId = recordsForGroup[0].farmId;
      
      // Sum up the volume (absolute value) for this period and farm
      const totalVolume = recordsForGroup.reduce((sum, record) => {
      ...
      for (const minerModel of MINER_MODELS) {
        const bitcoinMined = calculateBitcoinMined(totalVolume, minerModel, DEFAULT_DIFFICULTY);
...
async function updateMonthlySummary(date: string): Promise<void> {
  try {
    ...
    console.log(`\nUpdating monthly Bitcoin summary for ${month}...`);
    ...
    const monthlyResults = await db
      ...
      .where(sql`DATE_TRUNC('month', settlement_date) = DATE_TRUNC('month', ${date}::date)`)
    ...
    const deletedMonthly = await db
      ...
      .where(sql`DATE_TRUNC('month', month) = DATE_TRUNC('month', ${date}::date)`)
      .returning();
    ...
    for (const result of monthlyResults) {
      await db.insert(bitcoinMonthlySummaries).values({
        month: format(parsedDate, 'yyyy-MM-01'),
        minerModel: result.minerModel,
        totalBitcoinMined: result.totalBitcoin,
        lastUpdated: new Date()
      });
      
      console.log(`${result.minerModel}: ${parseFloat(result.totalBitcoin).toFixed(8)} BTC`);
      insertedMonthly++;
    }
    
    console.log(`Inserted ${insertedMonthly} monthly summary records for ${month}`);
  } catch (error) {
...
async function updateYearlySummary(date: string): Promise<void> {
  try {
    ...
    const yearlyResults = await db
      ...
      .from(historicalBitcoinCalculations)
      .where(sql`DATE_TRUNC('year', settlement_date) = DATE_TRUNC('year', ${date}::date)`)
      .groupBy(historicalBitcoinCalculations.minerModel);
    
    // Clear existing yearly records for this year and models
    const deletedYearly = await db
      .delete(bitcoinYearlySummaries)
      .where(sql`DATE_TRUNC('year', year) = DATE_TRUNC('year', ${date}::date)`)
      .returning();
    ...
    for (const result of yearlyResults) {
      await db.insert(bitcoinYearlySummaries).values({
        year: format(parsedDate, 'yyyy-01-01'),
        minerModel: result.minerModel,
        totalBitcoinMined: result.totalBitcoin,
        lastUpdated: new Date()
      });
