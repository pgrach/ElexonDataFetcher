/**
 * Bitcoin Calculation Updater Based on Daily Summaries
 * 
 * This script updates Bitcoin calculations for a specific date
 * based on daily summary totals rather than individual curtailment records.
 * 
 * Usage:
 *   npx tsx update_bitcoin_from_summaries.ts <date>
 * 
 * Example:
 *   npx tsx update_bitcoin_from_summaries.ts 2025-03-27
 */
...
function calculateBitcoinMined(energyMWh: number, minerModel: string, difficulty: number): number {
  ...
  // Formula: (petahashes * 1e15 / difficulty) * 6.25 BTC per block * number of seconds in a day
  const bitcoinMined = (petahashesGenerated * 1e15 / difficulty) * 6.25 * 86400;
...
async function processDate(date: string): Promise<void> {
  console.log(`\n=== Processing Bitcoin calculations from summaries for ${date} ===`);
  ...
  try {
    ...
    for (let period = 1; period <= periodsInDay; period++) {
      ...
      for (const minerModel of MINER_MODELS) {
        const bitcoinMined = calculateBitcoinMined(energyPerPeriod, minerModel, DEFAULT_DIFFICULTY) / periodsInDay;
