/**
 * Fix M20S Bitcoin Calculations for 2025-03-28 Missing Periods
 * 
 * This script will add the missing calculations for the M20S miner model
 */
...
const LOG_FILE = `./logs/fix_m20s_calculations_2025-03-28_${format(new Date(), 'yyyy-MM-dd')}.log`;
...
async function calculateBitcoinMining(): Promise<void> {
  try {
    log(`Starting M20S Bitcoin calculation updates for ${DATE_TO_PROCESS} missing periods ${MISSING_PERIODS.join(', ')}`);
      ...
      for (const farm of farms) {
        ...
        await db.execute(
          sql`INSERT INTO historical_bitcoin_calculations 
              ...
              VALUES 
              (${DATE_TO_PROCESS}, ${period}, ${farm.farm_id}, ${MINER_MODEL},
               ${btcMined}, ${difficulty}, NOW())`
        );
        
        log(`Added Bitcoin calculation for farm ${farm.farm_id} period ${period} with model ${MINER_MODEL}: ${btcMined.toFixed(8)} BTC`);
      }
    }
    
    // Count calculations for M20S model
    ...
    log(`Total Bitcoin calculations for ${MINER_MODEL}: ${calcCount}`, 'success');
...
async function main() {  
  try {
    log(`Starting M20S Bitcoin calculations for ${DATE_TO_PROCESS} missing periods ${MISSING_PERIODS.join(', ')}`, 'info');
    
    // Calculate Bitcoin mining for the new records
    await calculateBitcoinMining();
    
    log(`M20S Bitcoin calculations completed for ${DATE_TO_PROCESS} missing periods`, 'success');
  } catch (error) {
