...
async function loadBmuMappings(): Promise<{
  ...
  try {
    ...
    logger.info(`Loaded ${windFarmIds.size} wind farm BMU IDs`);
...
async function clearExistingPeriodsData(): Promise<void> {
  logger.info(`Clearing data for ${TARGET_DATE} periods ${START_PERIOD} to ${END_PERIOD}...`);
  ...
  try {
    ...
    logger.info(`Deleted ${deleteBitcoinResult.rowCount} Bitcoin calculation records`);
    ...
    logger.success(`Deleted ${deleteResult.rowCount} curtailment records for periods ${START_PERIOD}-${END_PERIOD}`);
  } catch (error) {
...
async function processPeriod(
  ...
  logger.info(`Processing period ${period} for ${TARGET_DATE}...`);
  ...
  try {
    ...
    logger.info(`[${TARGET_DATE} P${period}] Records: ${validRecords.length} (${validRecords.reduce((sum, r) => sum + Math.abs(r.volume), 0).toFixed(2)} MWh)`);
    
    if (validRecords.length === 0) {
      logger.warning(`[${TARGET_DATE} P${period}] No valid curtailment records found`);
    ...
    logger.success(`[${TARGET_DATE} P${period}] Total: ${periodTotal.volume.toFixed(2)} MWh, £${periodTotal.payment.toFixed(2)}`);
...
async function main(): Promise<void> {
  logger.info(`=== Staged Reingest for March 22, 2025 ===`);
  logger.info(`Started at: ${new Date().toISOString()}`);
  logger.info(`Target date: ${TARGET_DATE}`);
  logger.info(`Processing periods: ${START_PERIOD} to ${END_PERIOD}`);
  ...
  try {
    ...
    for (const period of periods) {
      ...
      logger.info(`Running total: ${totalRecords} records, ${totalVolume.toFixed(2)} MWh, £${totalPayment.toFixed(2)}`);
    ...
    logger.info(`- Processed periods ${START_PERIOD} to ${END_PERIOD}`);
    logger.info(`- Total records: ${totalRecords}`);
    logger.info(`- Total volume: ${totalVolume.toFixed(2)} MWh`);
    logger.info(`- Total payment: £${totalPayment.toFixed(2)}`);
    logger.info(`- Overall completion: ${completedPeriods.size}/48 periods`);
    ...
    if (missingPeriods.length > 0) {
      logger.warning(`Missing periods (${missingPeriods.length}): ${missingPeriods.join(', ')}`);
    } else {
      logger.success(`All 48 settlement periods are now complete!`);
    }
    
    logger.success(`\nReingest batch completed successfully at ${new Date().toISOString()}`);
    logger.info(`\nTo update summaries after all batches are complete, run:`);
    logger.info(`npx tsx update_march_22_summaries.ts`);
  } catch (error) {
    logger.error('Error during reingest process: ' + error);
    process.exit(1);
  } finally {
    await db.end();
  }
