...
async function deduplicateRecords(date: string, dryRun: boolean = true, batchSize: number = 50) {
  ...
  const duplicateKeys = [...farmPeriodMap.entries()]
  ...
  if (!dryRun) {
    ...
    console.log(`To execute the deduplication, run: npx tsx deduplicate_records.ts ${date} live`);
  }
}
