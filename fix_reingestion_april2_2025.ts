/**
 * Fixed Reingestion Script for April 2, 2025
 * 
 * This script performs a comprehensive reingestion of data for April 2, 2025:
 * 1. Clears all existing data for April 2, 2025 in curtailment_records and historical_bitcoin_calculations
 ...
 * 
 * Usage: npx tsx fix_reingestion_april2_2025.ts
 */
...
const LOG_FILE_PATH = `./logs/fixed_reingest_april2_2025_${format(new Date(), "yyyy-MM-dd'T'HH-mm-ss")}.log`;
  ...
  try {
    ...
    const yearlyStats = await db
      .select({
        totalCurtailedEnergy: sql<string>`SUM(total_curtailed_energy::numeric)::text`,
        totalPayment: sql<string>`SUM(total_payment::numeric)::text`,
        totalDays: sql<number>`SUM(total_days)::int` // Use total_days instead of non-existent avg_farm_count
      })
      .from(monthlySummaries)
      .where(sql`year_month::text LIKE ${year + '%'}`);
    
    if (yearlyStats.length > 0 && yearlyStats[0].totalCurtailedEnergy) {
      // Insert new yearly summary with correct column names
