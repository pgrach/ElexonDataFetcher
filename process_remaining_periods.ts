/**
 * Process Remaining Periods for 2025-03-28
 * 
 * This script processes the remaining periods (43-48) for 2025-03-28
...
const LOG_FILE = `process_2025-03-28_remaining_${new Date().toISOString().split('T')[0]}.log`;
const MAX_RETRIES = 3;
const RETRY_DELAY = 5000; // 5 seconds

// Date to process
const date = '2025-03-28';
const startPeriod = 43;
const endPeriod = 48;
  ...
  try {
    ...
    if (recordsToInsert.length > 0) {
      try {
        ...
        // Log summary instead of every record to avoid timeouts
...
async function processDate(): Promise<void> {
  try {
    ...
    // Initialize log file
    await fs.writeFile(LOG_FILE, `=== 2025-03-28 Remaining Periods Processing (${startPeriod}-${endPeriod}) ===\n`);
    ...
    // Process each period individually to avoid timeouts
    for (let period = startPeriod; period <= endPeriod; period++) {
      ...
      if (result.success) {
        ...
        // Update Bitcoin calculations for each period to avoid timeouts
        if (result.records > 0) {
          log(`Updating Bitcoin calculations for period ${period}...`, "info");
          for (const minerModel of ['S19J_PRO', 'S9', 'M20S']) {
            try {
              await processSingleDay(date, minerModel);
              log(`- Updated ${minerModel} calculations for period ${period}`, "success");
            } catch (error) {
              log(`- Error updating ${minerModel} calculations for period ${period}: ${error}`, "error");
            }
          }
        }
        
        // Add a longer delay between periods to avoid timeouts
        await delay(3000);
      }
    }
    
    // Final summary
    const successRate = (periodsProcessed / (endPeriod - startPeriod + 1)) * 100;
    
    log(`=== Processing complete for ${date} ===`, successRate === 100 ? "success" : "warning");
    log(`- Periods processed: ${periodsProcessed}/${endPeriod - startPeriod + 1} (${successRate.toFixed(1)}%)`, "info");
